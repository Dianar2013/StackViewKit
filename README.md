...
| 1、 | width | float | | 宽度 |
| 2、 | height | float | |  高度 |
| 3、 | partAlignment | string |center\\right\\left\\top\\bottom |   |
| 4、 | alignmentMargin | float | | 表示与assembleView的间距|
| 5、 | hugging | string |fit\\low\\high\\reqeired| 抗拉伸优先级越高越不容被拉伸|
| 6、 | compression | string |fit\\low\\high\\reqeired|  抗压缩优先级越高越不容被压缩|
| 7、 | minWidth | float ||最小宽度|
| 8、 | maxWidth | float ||最大宽度|
| 9、 | backColor | UIColor ||背景颜色|
| 10、 | backColorHexString | string ||背景色16进制值| 
| 11、 | backPaddingHorizontal | float ||partView与背景的水平距离|
| 12、 | backPaddingVertical | float ||partView与背景的垂直距离|
| 13、 | backBorderColor | UIColor ||背景的边框颜色|
| 14、 | backBorderColorHexString | string ||背景边框色16进制值|
| 15、 | backBorderWidth | float ||背景边框宽度|
| 16、 | backBorderRadius | float ||背景图圆角|
| 17、 | button | UIButton ||按钮|
| 18、 | buttonHighlightColor | UIColord ||按钮选择颜色|
| 19、 | text | string ||标签文本内容|
| 20、 | fontSize | float ||标签文本字体大小|
| 21、 | font | UIFont ||标签文本字号|
| 22、 | color | UIColor ||标签文本颜色|
| 23、 | colorHexString |  string ||标签文本颜色16进制值|
| 24、 | imageName |  string ||图片路径名称|
| 25、 | image |  UIImage ||图片对象|
| 26、 | imageUrl |  string ||图片网络下载路径|
